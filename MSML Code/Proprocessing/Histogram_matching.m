clear
path1='D:\Data\LUSC_Data\LUSC_ms\'; % File location of the cropped pathes 
files1=dir(path1);
Imatch=imread('Reference image.tiff');
for n=3:numel(files1)
    n
    path2=[path1,files1(n).name];
    path3=['D:\Data\LUSC_Data\LUSC_his\',files1(n).name,'\']; % File location of pathes after histogram matching 
    mkdir(path3);
    files2=dir(path2);
